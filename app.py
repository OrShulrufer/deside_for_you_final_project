...
predictions_df, main_clustering = Cluster.rearrange_clusters()

# trying threads
def task():
    threading.Timer(3600, task).start()
    if datetime.now().hour == 2:
        predictions_df, clustering = Cluster.rearrange_clusters()
...
def update_user_profile():
    ...
    if update_demographic:
        ...
        user_cluster_prediction = Cluster.predict_user_cluster(main_clustering, email)
        Cluster.update_user_cluster(user_cluster_prediction, email)
    #end=datetime.now()
