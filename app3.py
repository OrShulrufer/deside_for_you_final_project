...
conf = SparkConf()
conf.set('spark.scheduler.mode', 'FAIR') # for jobs
...
def get_or_create_SparkSession():
    return SparkSession.builder.appName("DecideForYou").config(conf=conf).getOrCreate() #Dan: maybe not use spark variable, but use getOrCreate()


# Create Spark Session
get_or_create_SparkSession().sparkContext.setLogLevel("WARN") #Dan: maybe not use spark variable, but use getOrCreate()
...
def task():
    ...
    if datetime.now().hour == 2:
        ...
        predictions_sparkdf = Cluster.rearrange_clusters(spark_session)
        Cluster.update_db_clusters(predictions_sparkdf, None, spark_session)
